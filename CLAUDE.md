...
There is no lint, test, or formatter setup. `npm run build` is the only automated check; run it before pushing. CI runs `npm ci` and the build on Node 20.
...
**Content lives in `src/data.js`; components only render it.** Profile, nav links, about text, experience, projects, certifications, skills, education and spoken languages are all exported from there. Text or data changes should go in `data.js`, not JSX.

Copy that is *not* sourced from `data.js` and must be updated by hand when profile details change:
- `index.html`: the `<title>`, meta description, Open Graph/Twitter tags and the JSON-LD `Person` block (employer, phone, city, `knowsAbout`), plus `public/og-image.png`.
- Section titles and leads are string literals passed to `Section` in each component. Two of them describe the data and go stale when it changes: the Projects lead ("Two of the three are live and open to try.") and the Certifications lead, which names the issuers.
- `Contact.jsx` hard-codes its heading, the "Open to AI engineering roles…" paragraph and the LinkedIn/GitHub handle text shown under each link. `Navbar.jsx` hard-codes the short mobile wordmark "A. R. Mihad".
...
- List entries are keyed by their own text (headline lines, competencies, stat `label`s, highlight `title`s, project `points`, tags, skill items, certification names, education `details`), so duplicates inside one list collide. Experience roles are keyed by `company` and projects by `name`, so a second role at the same company needs a different key.
...
Dark mode is class-based (`<html class="dark">`) and **light by default**: the inline script in `index.html` only adds the class when `localStorage['ui-theme'] === 'dark'`, and `useTheme` in `src/hooks.js` writes that key only when the visitor toggles (it also swaps the `theme-color` meta tag). Do not persist the theme on mount.
...
**Layout primitives** are in `src/components/ui.jsx`: `Section` renders the ruled 12-column header (index + label in the left three columns, title and optional lead in the remaining nine), requires `id` because it derives `aria-labelledby` from it, and takes `alt` for the grey `bg-alt` band (Skills uses it); content below usually starts at `lg:col-start-4` to align with the title. `Reveal` is the scroll fade-up (`delay` for staggering, `as` for the tag), `Counter` counts up on scroll, `Tag` is a chip, `Marquee` is the ticker. `Contact` hand-rolls its own `<section class="inverse">` instead of using `Section`.
...
**Code splitting:** `App.jsx` lazy-loads every section below `About` inside one `Suspense` whose fallback reserves 70vh so the page does not jump. `useActiveSection` uses a `MutationObserver` to re-measure once lazy chunks mount, so new lazy sections need no extra wiring beyond an `id` in `navLinks`.
